...
import (
	...
	perf "github.com/james-bebbington/winperfcounters"
...
func (pc *PerfCounter) ScrapeData() ([]perf.CounterValue, error) {
...
func removeTotalUnlessOnlyOneValue(vals []perf.CounterValue) []perf.CounterValue {
	if len(vals) <= 1 {
		return vals
	}
	...
	for i, val := range vals {
		if val.InstanceName == "_Total" {
			...
			vals[len(vals)-1] = perf.CounterValue{}
